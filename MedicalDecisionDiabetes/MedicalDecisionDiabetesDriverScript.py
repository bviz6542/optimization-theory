...
if __name__ == "__main__":
    ...
    labelsOutputPath = ["Policy", "Truth_type", "Theta", "Sample_path"] + mu_star_labels + ["Best_Treatment",
                                                                                            "n"] + mu_labels + sigma_labels + N_labels + [
                           "Decision", "W", "CumReward", "isBest"]

    output_path = []

    # data structures to accumulate best treatment count
    best_treat = {(p, theta): [] for p in policy_list for theta in
                  theta_range_1}  # one list for each (p,theta) pair - each list is along the sample paths
    best_treat_count_list = {(p, theta): [] for p in policy_list for theta in
                             theta_range_1}  # one list for each (p,theta) pair - the list is along the sample paths - each element of the list is accumulated during the experiments
    ...
    decision_Given_Best_Treat_list = {(p, theta, d): [] for p in policy_list for theta in theta_range_1 for d in
                                      x_names}
    decision_Given_Best_Treat_Counter = {(p, theta, d): [] for p in policy_list for theta in theta_range_1 for d in
                                         x_names}
        ...
        for theta in theta_range_1:
            ...
            for l in range(1, L + 1):
                ...
                for n in range(t_stop):
                    ...
                    record_sample_t = [n] + state_mu + state_sigma + state_N + [decision, exog_info['reduction'],
                                                                                model_copy.obj,
                ...
                last_state_dict.update({x: [last_state_dict[x][0] + getattr(model_copy.state, x)[0],
                                            last_state_dict[x][1] + getattr(model_copy.state, x)[1],
                                            last_state_dict[x][2] + getattr(model_copy.state, x)[2]] for x in x_names})
            ...
            print(
                "Finishing policy = {}, Truth_type {} and theta = {}. F_bar_mean = {:.3f} and F_bar_std = {:.3f}".format(
                    policy_chosen, Model.truth_type, theta, F_hat_mean, np.sqrt(F_hat_var / L)))
            states_avg = {x: [last_state_dict[x][0] / L, last_state_dict[x][1] / L, last_state_dict[x][2] / L] for x in
                          x_names}
            print("Averages along {} iterations and {} budget trial:".format(L, t_stop))
            for x in x_names:
                print(
                    "Treatment {}: m_bar {:.2f}, beta_bar {:.2f} and N {}".format(x, states_avg[x][0], states_avg[x][1],
                                                                                  states_avg[x][2]))
            ...
            decision_Given_Best_Treat_dict = {
                x: dict(normalizeCounter(Counter(decision_Given_Best_Treat_list[(policy_chosen, theta, x)]))) for x in
                Model.x_names}
    ...
    for policy_chosen, p in zip(policy_list, nPolicies):
        axsubs[0].plot(theta_range_1, theta_obj[policy_chosen], "{}o-".format(color_list[p]),
        ...
        axsubs[1].plot(theta_range_1, theta_obj_std[policy_chosen], "{}+:".format(color_list[p]),
                       label="{}".format(policy_chosen))
