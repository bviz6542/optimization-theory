...
class MedicalDecisionDiabetesModel():
    ...
    def __init__(self, state_names, x_names, S0, additional_params, exog_info_fn=None, transition_fn=None,
        ...
        if self.truth_type == 'fixed_uniform':
            self.truth_params_dict = {
                x: [S0.loc[x, 'mu_fixed'], S0.loc[x, 'fixed_uniform_a'], S0.loc[x, 'fixed_uniform_b']] for x in
                self.x_names}
        elif self.truth_type == 'prior_uniform':
            self.truth_params_dict = {x: [S0.loc[x, 'mu_0'], S0.loc[x, 'prior_mult_a'], S0.loc[x, 'prior_mult_b']] for x
                                      in self.x_names}
        else:
    ...
    def printState(self):
        ...
        for x in self.x_names:
            print("Treatment {}: mu {:.2f}, sigma {:.2f} and N {}".format(x, getattr(self.state, x)[0],
    ...
    def printTruth(self):
        ...
        for x in self.x_names:
            print("Treatment {}: par1 {:.2f}, par2 {:.2f} and par3 {}".format(x, self.truth_params_dict[x][0],
    ...
    def exog_info_sample_mu(self):
        if self.truth_type == "known":
            ...
            self.mu = {x: self.truth_params_dict[x][0] + self.prng.uniform(self.truth_params_dict[x][1],
                                                                           self.truth_params_dict[x][2]) for x in
                       self.x_names}
        elif self.truth_type == "prior_uniform":
            self.mu = {x: self.truth_params_dict[x][0] + self.prng.uniform(
                self.truth_params_dict[x][1] * self.truth_params_dict[x][0],
                self.truth_params_dict[x][2] * self.truth_params_dict[x][0]) for x in self.x_names}
        else:
            self.mu = {x: self.prng.normal(self.truth_params_dict[x][0], self.truth_params_dict[x][1]) for x in
                       self.x_names}
    ...
    def step(self, decision):
        ...
        current_state = {key: exog_info[decision] if key == decision else getattr(self.state, key) for key in
                         self.state_names}
