...
class MDDMPolicy():
    ...
    def UCB(self, model_curr, theta):
        ...
        aux_stats = {key: [getattr(model_curr.state, key)[0],
                           math.sqrt(math.log(model_curr.t + 1) / (getattr(model_curr.state, key)[2] + 1))] for key in
                     model_curr.state_names}
    ...
    def IE(self, model_curr, theta):
        ...
        stats = {key: getattr(model_curr.state, key)[0] + theta / math.sqrt(getattr(model_curr.state, key)[1]) for key
                 in model_curr.state_names}
