...
class GraphGenerator():
    ...
    def createNetworkSteps(self):
        ...
        for level in range(nSteps - 1):
            for i in nodesPerLevel[level]:
                G.add_node(i)
                edge_set = list(
                    self.prng.choice(nodesPerLevel[level + 1], min(3, len(nodesPerLevel[level + 1])), replace=False))
                for j in edge_set:
                    meanWeight = 1
    ...
    def get_deadline(self):
        return (self.init_args['costMin'] + (self.init_args['costMax'] - self.init_args['costMin']) * self.init_args[
            'deadlinePerc']) * (self.steps)
    ...
    def construct_network_objects(self, G, filename, start_node, end_node):
        ...
        for fromNode in range(size):
            ...
            for toNode in G.neighbors(fromNode):
                ...
                self.meanCosts[fromNode][toNode] = self.prng.uniform(self.init_args['costMin'],
                                                                     self.init_args['costMax'])
    ...
    def createNetworkChance(self):
        ...
        while done == 0:
            ...
            print("Iteration {}, Source {}, Dest {}, Length {}, number of paths {}".format(nbIterations, mSource, mDest,
                                                                                           maxLength, mPaths))
